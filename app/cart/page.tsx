...
export default function CartList(){
    ...
    const changeQuantity = async (commodityId:number, quantity:number) => {
        ...
        try{
            const response = await fetch(springUrl + `/cart/quantity/change?commodityId=${commodityId}&quantity=${quantity}`, {method:"post", credentials: "include"});
    ...
    const deleteCommodity = async (commodityId:number) => {
        ...
        try{
            const response = await fetch(springUrl + `/cart/order/delete?commodityId=${commodityId}`, {method:"post", credentials: "include"});
    ...
    return(
        <div className="min-h-screen bg-neutral-50 text-neutral-800 antialiased py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
                <h1 className="text-2xl font-bold tracking-tight text-neutral-900 mb-8 border-b border-neutral-200 pb-4">Shopping Cart</h1>
                ...
                <div className="bg-white rounded-xl border border-neutral-100 shadow-sm overflow-hidden mb-8">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-neutral-100 text-sm">
                            {orders.map(order => {
                                return(
                                    <tr key={order.commodityId} className="hover:bg-neutral-50/40 transition-colors duration-150">
                                        <td className="py-5 px-6 font-medium text-neutral-900">{order.menu.commodityName}</td>
                                        <td className="py-5 px-3 text-right text-neutral-600">¥{order.menu.price.toLocaleString()}</td>
                                        ...
                                        <td className="py-5 px-3 text-right font-semibold text-neutral-900">¥{(order.menu.price * order.quantity).toLocaleString()}</td>
                ...
                <div className="flex flex-col items-end gap-6 bg-white rounded-xl border border-neutral-100 shadow-sm p-6">
                    <div className="flex justify-between items-baseline w-full sm:w-80 border-b border-neutral-100 pb-4">
                        <span className="text-sm font-medium text-neutral-500">合計金額</span>
                        <span className="text-2xl font-bold text-neutral-900">¥{sumMonery.toLocaleString()}</span>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                        ...
                        <button 
                            onClick={() => {registCart(); registAlert();}}
                        ...
                        <button 
                            onClick={() => {purchase(); purchaseConfilm();}}
